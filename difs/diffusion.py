...
class GaussianDiffusionConditional(nn.Module):
    ...
    def model_predictions(self, x, t, cond, cond_scale=1., rescaled_phi=0.0, clip_x_start=False, rederive_pred_noise = False):
    ...
    @torch.no_grad()
    def p_sample(self, x, t: int, cond, cond_scale=1., rescaled_phi=0.0, clip_denoised=True):
    ...
    @torch.no_grad()
    def p_sample_loop(self, shape, cond, cond_scale=1., rescaled_phi=0.0):
    ...
    @torch.no_grad()
    def ddim_sample(self, shape, cond, cond_scale=1., rescaled_phi=0.0, clip_denoised=True):
    ...
    @torch.no_grad()
    def sample(self, cond, cond_scale=1., rescaled_phi=0.0):
